...
def add_stamp_item(
    stamp_pixmap: QPixmap, page_item: QGraphicsPixmapItem, position: QPointF
) -> QGraphicsPixmapItem:
    """
    ...
    stamp_item = QGraphicsPixmapItem(stamp_pixmap, page_item)

    # 1. 마우스가 찍히는 지점이 '도장'의 중앙이 위치되도록 삽입.
    stamp_center_x = stamp_pixmap.width() / 2
    stamp_center_y = stamp_pixmap.height() / 2
